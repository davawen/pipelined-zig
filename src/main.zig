...
const Tuple = std.ArrayList(Expr);
...
fn parse_expr(tokens: *Lexer) !Expr {
...
pub fn main() !void {
    const text: []const u8 = "{ (a b) => a |> print; \" + \" |> print; b |> print; \" = \" |> print; {a b} |> add |> print } -> print_add; {12 15} |> print_add";
    ...
    var lex = Lexer.init(tokens);
    const expr = try parse_expr(&lex);
